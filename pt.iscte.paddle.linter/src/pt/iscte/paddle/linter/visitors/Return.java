...
public class Return extends CodeAnalyser implements BadCodeAnalyser, IBlock.IVisitor {
...
//		}
		for (INode node : cfg.getNodes()) {
			...
			if (node instanceof IBranchNode && ((IBranchNode) node).hasBranch()) {
				...
				if (s != null && s.getGuard().getType().equals(IType.BOOLEAN) && s instanceof ISelection
						&& ((ISelection) s).hasAlternativeBlock()
					...
					if ((altRet.getExpression() != null && altRet.getExpression().isSame(IType.BOOLEAN.literal(true))
							&& nextRet.getExpression() != null
							&& nextRet.getExpression().isSame(IType.BOOLEAN.literal(false)))
					// || (altRet.getExpression() != null &&
					// altRet.getExpression().isSame(IType.BOOLEAN.literal(false))
					// && nextRet.getExpression() != null &&
					// nextRet.getExpression().isSame(IType.BOOLEAN.literal(true)))
					) {
	...
	public boolean visit(IReturn ret) {
		IProcedure proc = ret.getOwnerProcedure();
		if (!ret.isError() &&
			proc.getReturnType().isVoid() && 
			ret.getParent() == proc.getBlock() &&
			ret.getParent().getLast() == ret)
			issues.add(new UselessReturn(ret.getOwnerProcedure(), ret));
