...
public class CFGcases {
	
	public static IControlFlowGraph case1() {
	...
	public static IControlFlowGraph case2() {
		IModule module = IModule.create();
		IProcedure max = module.addProcedure(INT);
		IVariable array = max.addParameter(INT.array());
		IBlock body = max.getBody();
		...
		IStatementNode i_iInc = cfg.newStatement(iInc);
		cfg.getEntryNode().setNext(i_iInc);
		
		IStatementNode s_ret = cfg.newStatement(ret);
		i_iInc.setNext(s_ret);
